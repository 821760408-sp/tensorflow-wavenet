...
from wavenet import (WaveNetModel, time_to_batch, batch_to_time, conv1d,
...
def make_sine_waves(global_conditioning):
    ...
    times = np.arange(0.0, SAMPLE_DURATION, SAMPLE_PERIOD_SECS)
    ...
    if global_conditioning:
        ...
        start_time = LEADING_SILENCE * SAMPLE_PERIOD_SECS
...
def check_waveform(assertion, generated_waveform, gc_category):
    librosa.output.write_wav('data_tmp/sine_test{}.wav'.format(gc_category),
                             generated_waveform,
                             SAMPLE_RATE_HZ)
